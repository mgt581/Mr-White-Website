...
async function ensureLeadEventSchema(db) {
  ...
  var schema = await db.prepare("PRAGMA table_info(lead_events)").all();
  var columns = new Set((schema.results || []).map(function(item) { return item.name; }));
  if (!columns.has("email_address")) {
    await db.prepare("ALTER TABLE lead_events ADD COLUMN email_address TEXT").run();
  }
